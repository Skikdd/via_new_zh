    ...
    <>
      <AutoHeightRow>
        <ReactTextareaAutocomplete
          ...
          placeholder={`输入你希望 M${macroIndex} 执行的宏...`}
          trigger={{
            '?': {
      ...
      <AutoHeightRow>
        <DescriptionLabel>
          ...
          <Message>按下一个键 {'{KC_XXX}'}</Message>
          <Message>同时按下多个键 {'{KC_XXX, KC_YYY, KC_ZZZ}'}</Message>
          <Message>按下按键: {`{+KC_XXX}`}</Message>
          <Message>松开按键: {`{-KC_XXX}`}</Message>
          {isDelaySupported ? (
            <Message>Delay (ms): {'{NNNN}'} </Message>
          ) : (
            '升级固件以使用延迟功能'
          )}
          <Message>输入?可以弹出键码搜索框</Message>
        </DescriptionLabel>
        <Detail>
